...
def load_setting():
    settings = {
        ...
        'Tumor extension': {'values': ["Localized", "No vascular invasion", "Further extension"],
                            'type': 'selectbox', 'init_value': 1, 'add_after': ''},
        'Number of tumors': {'values': ["Only one", "More"], 'type': 'selectbox',
                               'init_value': 0,
                               'add_after': ''}
    }
    input_keys = ['Age', 'Chemotherapy', 'Gender', 'Grade', 'Marital status','Number of tumors',
       'Primary site', 'Race', 'Radiotherapy', 'Surgery', 'Tumor extension', 'Tumor size']
